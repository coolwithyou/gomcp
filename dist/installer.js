...
export async function installServers(serverIds, configs, scope = 'user', force = false) {
...
function getWarningMessage(server, scope) {
    const warnings = {
        postgresql: {
    ...
    return (warnings[server.id]?.[scope] || `${server.name} works best at ${server.preferredScope} level`);
}
