...
import { displayActivationStatus, getProjectServersActivationStatus, activateServers, deactivateServers, hasClaudeSettings, initializeClaudeSettings, } from './activation.js';
...
export async function mainMenu(defaultScope = 'user', mode, showDescriptions) {
    if (mode === 'remove') {
...
async function getInstalledServersByScope() {
...
async function installFlow(defaultScope = 'user', showDescriptions) {
    ...
    for (const category of categories) {
        ...
        for (const server of categoryServers) {
            ...
            const isInstalledInSelectedScope = scope === 'user'
                ? installedServers.user.has(server.id)
                : installedServers.project.has(server.id);
            const isInstalledInOtherScope = scope === 'user'
                ? installedServers.project.has(server.id)
                : installedServers.user.has(server.id);
    ...
    if (confirm) {
        await installServers(selectedServers, configs, scope, false);
        clearInstalledServersCache();
    }
    else {
        console.log(chalk.yellow('\nInstallation cancelled.'));
    }
}
async function collectServerConfig(server) {
    const config = {};
...
async function promptForOption(option) {
    const basePrompt = {
        name: 'value',
...
async function removeFlow(_defaultScope = 'user') {
    const progressBar = createIndeterminateProgressBar({
    ...
    const userServers = selectedServers.filter((s) => s.scope === 'user').map((s) => s.id);
...
async function activationManagementFlow() {
    ...
    console.log(`All project servers enabled: ${status.enableAllProjectMcpServers ? chalk.green('✓ Yes') : chalk.red('✗ No')}`);
...
async function specificServerActivationFlow(serverStatuses) {
...
async function permissionsManagementFlow(serverStatuses) {
    ...
    switch (action) {
        case 'add':
            const serversWithoutWildcard = serverStatuses.filter((s) => !status.permissions.includes(`mcp__${s.id}__*`));
...
async function deactivationFlow(serverStatuses) {
